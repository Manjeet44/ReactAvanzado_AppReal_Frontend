...
const ACTUALIZAR_PEDIDO = gql`
    mutation actulizarPedido($id: ID!, $input: PedidoInput){
        actulizarPedido(id: $id, input: $input) {
            estado
        }
    }
`;
...
const Pedido = ({pedido}) => {
    ...
    const [actulizarPedido] = useMutation(ACTUALIZAR_PEDIDO);
    ...
    const cambiarEstadoPedido = async nuevoEstado => {
        try {
            const {data} = await actulizarPedido({
                variables: {
                    id,
                    input: {
                        estado: nuevoEstado,
            ...
            setEstadoPedido(data.actulizarPedido.estado);
        } catch (error) {
            console.log(error)
        }
    }
